...
$(function() {
	...
	if (errorinfo.val() == null || errorinfo.val() == "") {
		file.next().html("*上传文件大小不能超过5M*上传文件类型必须为:jpg、jpeg、png、pneg");
	} else {
		file.next().html(errorinfo.val());
	}

	file = $("#files2");
	errorinfo = $("#errorinfo_wp");
	if (errorinfo.val() == null || errorinfo.val() == "") {
		file.next().html("*上传文件大小不能超过5M*上传文件类型必须为:jpg、jpeg、png、pneg");
	} else {
		file.next().html(errorinfo.val());
	...
	userCode.bind("blur", function() {
		// ajax后台验证--userCode是否已存在 user.do?method=ucexist&userCode=**
		// alert("blur");
		$.ajax({
			type : "GET",// 请求类型
